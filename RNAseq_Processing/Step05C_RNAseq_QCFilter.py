...
def compute_qc_metrics(counts: pd.DataFrame,
    ...
    # Median log2(TPM+1) proxy: use raw counts / total * 1e6 as pseudo-TPM
    log2counts = np.log2(counts + 1)
    expressed  = counts > 0
    m["Median_log2CPM"] = pd.Series(
        {col: float(log2counts.loc[expressed[col], col].median())
...
def main():
    ...
    if groups_df is not None:
        # groups_df index has prefixed names; keep_cols are bare — reindex safely
        _grp_bare = groups_df.copy()
        _grp_bare.index = [i.split("__")[-1] if "__" in str(i) else str(i)
                           for i in _grp_bare.index]
        _grp_bare = _grp_bare[~_grp_bare.index.duplicated(keep="first")]
        _grp_bare.loc[[c for c in keep_cols if c in _grp_bare.index]].reset_index().rename(
            columns={"Embryo": "Embryo", "index": "Embryo"}
        ).to_csv(
