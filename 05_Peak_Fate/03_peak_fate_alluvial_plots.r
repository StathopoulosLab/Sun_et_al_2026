...
#
# grpA/grpB below are the only condition-specific inputs this script needs —
# swap them (and out_contrast's subfolder) for any other two-genotype,
# two-timepoint contrast; the plotting logic itself is not vent-specific.
################################################################################

out_dir  <- "./Overview_Plots"
out_contrast <- file.path(out_dir, "peak_fate", "BOTv_vs_BOTCv")  # <- contrast-specific subfolder; rename per comparison
...
cat("Loading fate table from plot_peak_fate_v11.r output...\n")
...
if (!file.exists(fate_csv_path))
  stop("peak_fate_data.csv not found at ", fate_csv_path,
       " -- run plot_peak_fate_v11.r first. It is now the single source of ",
       "truth for fate classification; this script only reshapes its output.")
